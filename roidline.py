...
params = [
    ...
    # ask the user whether or not he wishes to view them; otherwise, they are
    # simply listed. This variable must be set to an integer value greater
    # than or equal to 0. A negative value means Readline should never ask.
    ...
    # without subsequently executing the character as a command (see
    # Searching). If this variable has not been given a value, the characters
    ...
    # Readline will use the shorter but complete key sequence. Readline uses
    # this value to determine whether or not input is available on the current
    # input source (rl_instream by default). The value is specified in
    # milliseconds, so a value of 1000 means that Readline will wait one
    # second for additional input. If this variable is set to a value less
    # than or equal to zero, or to a non-numeric value, Readline will wait
    ...
    # user in the filename to be completed. This variable is 'on' by default.
    ...
    # insertion ('+'). The default value is 'off'.
    ...
    # when the cursor is after the 'e' in 'Makefile' will result in 'Makefile'
